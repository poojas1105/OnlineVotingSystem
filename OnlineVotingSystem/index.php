		...
		<div class="d-flex justify-content-center h-100">
			<div class="user_card">
						...
						<div class="d-flex justify-content-center form_container">
								<form method="POST">
									...
									<div class="input-group mb-2">
										...
										<input type="password" name="su_re-enter_your_password" class="form-control input_pass"  placeholder="Re-enter Your Password" required />
									</div>
									
										<div class="d-flex justify-content-center mt-3 login_container">
								<button type="submit" name="sing_up_btn" class="btn login_btn">Sing Up</button>
							</div>
								</form> 
							</div>
				...
				?>
						...
						<div class="mt-4">
							<div class="d-flex justify-content-center links text-white">
								Don't have an account? <a href="?sing-up=1" class="ml-2 text-white">Sign Up</a>
							</div>
				...
				<?php
					...
					{
				?>
						<span class="bg-white text-success text-center my-3"> Your account has been created succesfully! </span>
				<?php
...
<?php
	...
	if(isset($_POST['sing_up_btn']))
	{
		...
		$su_reenter_your_password = mysqli_real_escape_string($db, sha1($_POST['su_re-enter_your_password']));
		$user_role = "Voter";

		if($su_password == $su_reenter_your_password)
		{
			//  Insert Query
		...
		?>
			<script> location.assign("index.php?sing-up=1&registered=1"); </script>
		<?php	
		
		
		} else {
	?>
			<script> location.assign("index.php?sing-up=1&invalid=1"); </script>
	<?php
		}
	...
	} else if(isset($_POST['loginBtn']))
	{
		...
		// Query Fetch / SELECT
		$fetchingDatta = mysqli_query($db, "SELECT * FROM users WHERE contact_no = '". $contact_no ."' ") or die(mysqli_error($db));

		if(mysqli_num_rows($fetchingDatta) > 0)
		{
			$data = mysqli_fetch_assoc($fetchingDatta);
	...
	?>
			<script> location.assign("index.php?sing-up=1&not_registered=1"); </script>
	<?php
		}
	}
